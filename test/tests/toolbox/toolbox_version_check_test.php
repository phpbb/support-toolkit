...
class toolbox_version_check_test extends stk_test_case
{
	...
	public function test_getVersionInfo()
	{
		...
		$this->assertEquals($expected, $this->stk['vc']->getVersionData());
	}
	...
	public function test_getVersionInfo_category()
	{
		...
		$this->assertEquals($expected, $this->stk['vc']->getVersionData('foo'));
	}

	public function test_getVersionInfo_tool()
	{
		$expected = unserialize('O:8:"stdClass":2:{s:6:"latest";s:5:"1.0.0";s:8:"blocking";a:0:{}}');
		$this->assertEquals($expected, $this->stk['vc']->getVersionData('foo', 'foobar'));
	}

	public function test_no_version_entry()
	{
		$this->assertSame(stk_core_version_controller::VERSION_OK, $this->stk['vc']->testToolVersion('foo', 'barfoo'));
	}

	public function test_version_warning()
	{
		$this->assertSame(stk_core_version_controller::VERSION_WARNING, $this->stk['vc']->testToolVersion('foo', 'foobar'));
	}

	public function test_version_blocking()
	{
		$this->assertSame(stk_core_version_controller::VERSION_BLOCKING, $this->stk['vc']->testToolVersion('foo', 'bar'));
	}

	public function test_version_disabled()
	{
		$this->assertSame(stk_core_version_controller::VERSION_DISABLED, $this->stk['vc']->testToolVersion('foo', 'disabled'));
	}
