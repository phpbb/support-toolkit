	...
	function run_tool(&$error)
	{
		...
		foreach ($non_conflicted as $user)
		{
			...
			$sql = 'UPDATE ' . POSTS_TABLE . "
				...
				WHERE post_username = '{$user}'";
			$db->sql_query($sql);

			// Update topics table (first post)
			$sql = 'UPDATE ' . TOPICS_TABLE . "
				SET topic_poster = {$user_id}, topic_first_poster_colour = '{$gcl}'
				WHERE topic_first_poster_name = '{$user}'";
			$db->sql_query($sql);

			// Update topics table (last post)
			$sql = 'UPDATE ' . TOPICS_TABLE . "
				SET topic_last_poster_id = {$user_id}, topic_last_poster_colour = '{$gcl}'
				WHERE topic_last_poster_name = '{$user}'";
			$db->sql_query($sql);

			// Update forums table
			$sql = 'UPDATE ' . FORUMS_TABLE . "
				SET forum_last_poster_id = {$user_id}, forum_last_poster_colour = '{$gcl}'
				WHERE forum_last_poster_name = '{$user}'";
			$db->sql_query($sql);
